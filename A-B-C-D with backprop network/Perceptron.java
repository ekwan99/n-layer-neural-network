...
* @version 5/1/24
* 
* The Perceptron class contains code for an A-B-C-D network, which is a network configured to have 
* A input nodes, B hidden layer nodes, and C nodes in the second hidden layer, and D output nodes. 
...
*     public static void runCurrentNetwork(int t)
...
*/ 
   private static int numHiddenLayers;         // number of layers in network
   private static int numActivationLayers;
   private static int numIn;             // number of input activations
   private static int numOut;            // number of output activations
   private static int[] activationsPerLayer;
   ...
   private static int m, i, j, k;        // iterators of the layers--input, output, first hidden, and second hidden layers, respectively
   private static int n;                 // index of layer of model
...
*/
   ...
   private static String testingDataFile;    // name of the file that contains testing data activation values
...
*/
   ...
   private static double[][] psiArray;    // array of saves psi values
...
*/
   ...
   private static double totalError;       // calculated total error across test cases
...
/* 
 * Declaration of different possible constant values of n (determines index that represents layer of the model)
 */
   static final int INPUTLAYER = 0;
   static final int HIDDENLAYER1 = 1;
   static final int HIDDENLAYER2 = 2;
 ...
 */
         numHiddenLayers = Integer.parseInt(br.readLine().split("= ")[1]);
         numActivationLayers = numHiddenLayers + 1;

         String[] readInActivationsPerLayer = br.readLine().split("= ")[1].split("-");
         activationsPerLayer = new int[numActivationLayers + 1];
         for (n = 0; n < numActivationLayers; n++)
         {
            activationsPerLayer[n + 1] = Integer.parseInt(readInActivationsPerLayer[n]);
         }
         numIn = activationsPerLayer[1];
         numOut = activationsPerLayer[numActivationLayers];
...
*/
   public static void allocateArrayMemory()
   {
      maxDimension = findMaxDimension();

      a = new double[numActivationLayers + 1][maxDimension];
      
      w = new double[numActivationLayers][maxDimension][maxDimension];
      ...
      try (BufferedReader bReader = new BufferedReader(new FileReader(testingDataFile)))
      {
         ...
         for (int t = 0; t < numTests; t++)
         {
            ...
            for (m = 0; m < numIn; m++)
            {
               ...
               // DEBUG: System.out.println("Testing data: " + testingData[t][k]);
            }
         } // for (int t = 0; t < numTests; t++)
 ...
 */
         for (int t = 0; t < numTests; t++)
         {
            ...
            for (int i = 0; i < numOut; i++)
            {
               ...
               // DEBUG: System.out.println("Testing expected: " + testingExpected[t][i]);
            }
         } // for (int t = 0; t < numTests; t++)
...
*/
   public static void saveWeights()
   {
      ...
      {
         writer.print("These are the weights for a " + numHiddenLayers + " layer network with a "); 
         for (int n = 1; n < numActivationLayers; n++)
         {
...
/*
* Saves weights between input and hidden layer 1
*/
         for (int n = 0; n < numActivationLayers; n++)
         {
            for (int k = 0; k < maxDimension; k++)
...
/*
* Loads weights between input and hidden layer 1
*/
 ...
 */
   public static void setUp(int t1)
   {
      for (m = 0; m < numIn; m++)  
      {
         ...
         System.out.println("a[0][" + m + "] = testingData[" + t1 + "][" + m + "] = " + testingData[t1][m]); 
      }
   } // setUp()
...
/*
* Calculates omega and psi values for first and second layers to implement the backpropagation algorithm.
*/

            double omegaVal;

            for (n = numHiddenLayers; n > 1; n--)
            {

               for (j = 0; j < activationsPerLayer[n]; j++)
               {
                  omegaVal = 0.0;
                  for (k = 0; k < activationsPerLayer[n + 1]; k++)
                  {
                     omegaVal = psiArray[n][k] * w[n][j][k];
                     w[n][j][k] += lambda * a[n][j] * psiArray[n][k];
                  }
                  psiArray[n - 1][j] = omegaVal * activationFunctionPrime(thetaArray[n - 1][j]);
 ...
 */
            ...
            for (j = 0; j < activationsPerLayer[1]; j++)
            {
               omegaVal = 0.0;

               for (k = 0; k < activationsPerLayer[2]; k++) 
               {
                  omegaVal += psiArray[1][k] * w[1][j][k];
                  w[1][j][k] += lambda * a[1][j] * psiArray[1][k];;
               }

               psiArray[0][j] = omegaVal * activationFunctionPrime(thetaArray[0][j]);

               for (m = 0; m < numIn; m++)
               {
                  w[0][m][j] += lambda * a[0][m] * psiArray[0][j];
               }
 ...
 */
            ...
            for (i = 0; i < numOut; i++)
            {
               omegaI = testingExpected[t][i] - a[numHiddenLayers][i];
               totalError += 0.5 * omegaI * omegaI;
            }
         } // for (int t = 0; t < numTests; t++)
...
*/
   public static void run()
   {
      for (int t = 0; t < numTests; t++) 
      {
         ...
         for (int i = 0; i < numOut; i++)
         {
            networkOutput[t][i] = a[numActivationLayers][i];
         }
      } // for (int t = 0; t < numTests; t++) 
   } // run()

/*
* Runs the model, calculating and saving thetaK, thetaJ, and thetaI values.
*/
   public static void runForTrain(int t1)
   {
      for (n = 1; n < numActivationLayers; n++)
      {
         for (j = 0; j < activationsPerLayer[n]; j++)
         {
            thetaArray[n][j] = 0.0;
...
/* 
* Calculates the activations, theta, omega, and psi values for output layer
*/ 

      for (i = 0; i < numOut; i++)
      {
         thetaArray[numHiddenLayers][i] = 0.0;

         for (j = 0; j < activationsPerLayer[numHiddenLayers - 1]; j++) 
         {
            thetaArray[numHiddenLayers][i] += a[numHiddenLayers - 1][j] * w[numHiddenLayers - 1][j][i];
         }

         a[numHiddenLayers][i] = activationFunction(thetaArray[numHiddenLayers][i]);
         psiArray[numHiddenLayers][i] = (testingExpected[t1][i] - a[numHiddenLayers][i]) * 
                                          activationFunctionPrime(thetaArray[numHiddenLayers][i]);
      } // for (i = 0; i < numOut; i++)
   } // runForTrain(int t1)

/*
 * Runs the network and calculates thetaK, thetaJ, and thetaK values, but doesn't save them.
 */
   public static void runCurrentNetwork(int t)
   {
/* 
* Calculates theta values for the first hidden layer and sets the hidden layer 1 activation values
*/   
      ...
      for (int n = 1; n <= numActivationLayers; n++)
      {
         for (int j = 0; j < activationsPerLayer[n]; j++)
         {
            ...
            System.out.println("a[" + n + "][" +j +  "] = " + a[n][j]);
         }
      }

      /* for (int j = 0; j < activationsPerLayer[numActivationLayers]; j++)
      {
         thetaVal = 0.0;

         for (int k = 0; k < activationsPerLayer[numActivationLayers - 1]; k++) 
         {
            thetaVal += a[numActivationLayers - 1][k] * w[numActivationLayers - 1][k][j];
         }
         a[numActivationLayers][j] = activationFunction(thetaVal);
...
*/
      ...
      for (int t = 0; t < numTests; t++)
      {
         ...
         for (i = 0; i < numOut; i++)
         {
            System.out.printf("%.17f | ", networkOutput[t][i]);
         }
